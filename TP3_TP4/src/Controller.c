...
int controller_ListEmployee(LinkedList* pArrayListEmployee)
{
    ...
    {
    	retorno = 0;
    	ll_map(pArrayListEmployee, employee_print);
    }
	return retorno;
}
 ...
 *
 */
int controller_reportsEmployee(LinkedList* pArrayListEmployee)
{
    int retorno = -1;
    LinkedList* listaEmpleadosQuePaganGanancia = NULL;
    int menuReportsOpcion;
    char menuReports[MENUREPORTS_LEN][300] = {
    			"Seleccione el informe necesario?",
    			"Empleados que pagan Ganancias (Con sueldo mayor a $30000)",
				"Sueldo total a pagar de Empleados con Jornada laboral extendida (Mayor a 120 hs Mensuales)",
				"Cantidad de Empleados que ganan mas de $60000"
    			};

    if(pArrayListEmployee != NULL)
    {

    	menuReportsOpcion = menu(menuReports,MENUREPORTS_LEN);

        switch(menuReportsOpcion)
        {
            case 1:
            	listaEmpleadosQuePaganGanancia = ll_filter(pArrayListEmployee, employee_isSueldoMayor30000);
            	if (ll_len(listaEmpleadosQuePaganGanancia) > 0)
            	{
					printf ("\nEl filtro arrojo como resultado %d empleados", ll_len(listaEmpleadosQuePaganGanancia));
					if(utn_isConfirmacion("\nDesea imprimir la lista filtrada? (S o N): ", "\nERROR\n", "S", "N", 3))
					{
						ll_map(listaEmpleadosQuePaganGanancia, employee_print);
					}
            	}
            	else
            	{
            		printf ("\nEl filtro no arrojo ningun resultado");
            	}
            	break;
            case 2:
            	printf("\nEl sueldo total de los mayores a 120 hs Mensuales es: $%d",ll_reduceAdding(pArrayListEmployee, employee_getSueldoEnHorasMayorA120));
            	break;
            case 3:
            	printf("\nLa cantidad de empleados que cobran mas de $60000 es: %d",ll_reduceAdding(pArrayListEmployee, employee_isSueldoMayorA60000));
            	break;
        }
